The Accessibility module provides WCAG 2.1 AA compliance features that are automatically initialized when Global Banking Empowerment loads.
...
The module is initialized in `Global Banking Empowerment.ts` with all features enabled by default:
